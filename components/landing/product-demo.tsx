...
export function ProductDemo() {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-primary-foreground/15 bg-card text-card-foreground shadow-2xl">
      <div
        className="flex overflow-x-auto border-b border-border px-3 pt-3"
        role="tablist"
...
function Overview() {
  const items = [
    ['Em execução', '8'],
    ['Aguardando revisão', '6'],
    ['Com pendências', '3'],
    ['Prontas para faturar', 'R$ 14.850'],
  ]
  return (
...
function Orders() {
  const rows = [
    ['#1842', 'Clima Norte', 'Marcos', 'Revisão', 'R$ 1.480'],
    ['#1841', 'Solaris Energia', 'Aline', 'Em execução', 'R$ 920'],
    ['#1839', 'Hospital Vida', 'João', 'Pendente', 'R$ 2.150'],
  ]
  return (
    <div className="flex flex-col gap-3">
      {rows.map((r) => (
        <div
          key={r[0]}
          className="grid gap-2 rounded-xl border border-border p-4 text-sm sm:grid-cols-5 sm:items-center"
        >
          <strong>{r[0]}</strong>
          <span>{r[1]}</span>
          <span>{r[2]}</span>
          <span className="text-muted-foreground">{r[3]}</span>
          <span className="font-semibold sm:text-right">{r[4]}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex gap-3 rounded-xl border border-warning/40 bg-warning/10 p-4 text-sm">
        ...
        <span>
          <strong>Pendência:</strong> falta fotografia da identificação do
          equipamento.
        </span>
      </div>
    </div>
...
function Billing() {
  return (
    <div className="flex flex-col gap-3">
      {[
        ['Clima Norte', '#1842', '10 ago. 2026', 'R$ 1.480'],
        ['Edifício Central', '#1837', '09 ago. 2026', 'R$ 3.250'],
        ['Solaris Energia', '#1832', '08 ago. 2026', 'R$ 2.090'],
      ].map((r) => (
        <div
          key={r[1]}
